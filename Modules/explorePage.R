...
explorePageServer <- function(id, reactiveData) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$generate, {
      ...
      inputs$startDate <- ifelse(is.na(input$selectedDates[1]), min(data$Date), input$selectedDates[1])
      inputs$endDate <- ifelse(is.na(input$selectedDates[2]), max(data$Date), input$selectedDates[2])
