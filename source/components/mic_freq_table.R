create_mic_frequency_tables <- function(
  ...
  MIC_table_long <- data %>%
    ...
    tidyr::pivot_wider(names_from = .data$MIC, values_from = .data$n, values_fill = 0) %>%
    {
      ...
      df %>%
        dplyr::arrange(!!rlang::sym(group_by_var)) %>%
  ...
  if (!is.na(bp_s) && !is.na(bp_r)) {
    ...
    get_color <- function(label, bp_s, bp_r) {
      ...
      if (grepl("^<\\d+(\\.\\d+)?$", label)) {
        ...
        if (x <= bp_s) "#44CDC4" else "white"
      } else if (grepl("^<=\\d+(\\.\\d+)?$", label)) {
        x <- as.numeric(sub("^<=", "", label))
        if (x <= bp_s) "#44CDC4" else "white"
      } else if (grepl("^>\\=?\\d+(\\.\\d+)?$", label)) {
        x <- as.numeric(gsub("[^0-9.]", "", label))
        if (x >= bp_r) "#D73027" else "white"
      } else if (grepl("^\\d+(\\.\\d+)?$", label)) {
        x <- as.numeric(label)
        if (x <= bp_s) {
          "#44CDC4"
    ...
    for (col in names(mic_colours)) {
      MIC_gt <- MIC_gt %>%
        gt::data_color(columns = dplyr::all_of(col), fn = function(x) {
          rep(mic_colours[[col]], length(x))
        })
    }

    legend_html <- gt::html(
      paste0(
        '<span style="background:#44CDC4;padding:0.15em 0.7em;margin:0 0.7em;border-radius:3px;"> </span>S (≤ ',
        bp_s,
        ')&nbsp;&nbsp;',
        '<span style="background:#FEE08B;padding:0.15em 0.7em;margin:0 0.7em;border-radius:3px;"> </span>I (',
        bp_s,
        '-',
        bp_r,
        ')&nbsp;&nbsp;',
        '<span style="background:#D73027;padding:0.15em 0.7em;margin:0 0.7em;border-radius:3px;"> </span>R (≥ ',
        bp_r,
        ')',
